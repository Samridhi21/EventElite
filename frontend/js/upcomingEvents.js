document.addEventListener("DOMContentLoaded", () => {
    ...
    function createEventCard(event) {
        ...
        // image path
        let imageUrl = event.image || "/uploads/default-event.jpg"; // Use default image if null
...
if (!imageUrl.startsWith("http")) {
    imageUrl = `http://localhost:3000${imageUrl.startsWith("/uploads/") ? imageUrl : `/uploads/${imageUrl}`}`;
        ...
        eventCard.innerHTML = `
           ...
           <button class="enroll-btn" data-event-id="${event.event_id}" 
                    data-title="${event.title}" 
                    data-date="${event.start_date.split("T")[0]}" 
                    data-time="${event.time}" 
                    data-venue="${event.venue}" 
                    data-fee="${event.fee}">Enroll Now</button>
        `;
